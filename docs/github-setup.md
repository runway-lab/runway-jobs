...
suggests: it requires **every commit on a PR**, not just the merge
...
.commit.verification` — `verified: true, reason: valid` will show up.
The protection we lose by not enforcing is "block someone from
manually pushing an unsigned commit to main" — and branch protection
already blocks all direct push to main, so this is redundant.
